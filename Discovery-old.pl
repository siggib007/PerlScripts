...
use English;
use Sys::Hostname;
my ($comstr,$session,$error,$sysname, $sysDescr, $result, $numin, $logfile, $errmsg, $progname, $t, $seq, $City );
my ($sname, $sDescr, $strOut, $InFile, $outfile, $IPAddr, $line, $bError, $alive, $PortTest, $SSHTest, @body);
my ($DNSname, $iaddr, $Subnet, $Q1, $Q2, $Q3, $Q4, $devType, $i, $StartTime, $StopTime, $relay, $to, $subject);
my ($isec, $imin, $ihour, $iDiff, $iDays, $iDiff, $iHours, $iDiff, $iMins, $iSecs, $TelnetTest, $from, $Market);
...
$strOut = "IP\tDNS_Name\tSysName\tSysDescr\tType\tTelnet\tSSH\n";
logentry ("$strOut");
print OUT $strOut;
...
foreach $line (<IN>)
{
	...
	{
		$IPAddr = $Q1 . "." . $Q2 . "." . $Q3 .  "." . $i;
		logentry("processing address $IPAddr for $Market in $City...\n");
		...
		if ($alive)
		{
  		...
  		if (! defined $DNSname) 
  		{
			...
			if (!defined($session)) 
			{
				...
				logentry ("$errmsg\n");
				$strOut = "$IPAddr\t$DNSname\t$errmsg\t\t\t$TelnetTest\t$SSHTest\n";
				logentry ("$strOut");
				print OUT $strOut;
				$bError = 1;
			}
...
#					logentry ("Sysname: $sname\n");
		        ...
		        logentry ("$errmsg");
			      $strOut = "$IPAddr\t$DNSname\t$sname\t$error\t\t$TelnetTest\t$SSHTest\n";
			      logentry ("$strOut");
	      	  print OUT $strOut;
	      	  $bError = 1;
			   	}
					...
					$sDescr =~ s/\t/ /g;
					$sDescr =~ s/\r/ /g;
					($devType) = split (/ /, $sDescr);
					logentry ("SysDescr: $sDescr\n");
		  	}
			}
			if ($bError == 0)
			{
				$strOut = "$IPAddr\t$DNSname\t$sname\t$sDescr\t$devType\t$TelnetTest\t$SSHTest\n";
				logentry ("$strOut");
				print OUT $strOut;		  
			}		
			$session->close;
