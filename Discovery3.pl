...
use English;
use Sys::Hostname;
my ($comstr, $session, $error, $sysname, $sysDescr, $result, $logfile, $errmsg, $progname, $t, $quad, $Q3);
my ($sname, $sDescr, $strOut, $InFile, $outfile, $IPAddr, $line, $bError, $alive, $PortTest, $SSHTest, $x, $a);
my ($DNSname, $iaddr, $Subnet, $SubnetEnd, @subnetquads, $devType, $i, $StartTime, $StopTime, $relay, $to);
my ($isec, $imin, $ihour, $iDiff, $iDays, $iDiff, $iHours, $iDiff, $iMins, $iSecs, $TelnetTest, $Perc, $pwd);
my ($from, $Make, @strSplit, $City, $Market, $subject, @body, $CurrIP, $seq, $TotalIP, $HTTPTest, @tmpArray);
my ($HTTPSTest, $InUse, %SMTPHosts, $key, $value, $relayname, $host, $verbose, $bValid, $goodpar, @lines, @inArray);
...
print "It will perform the following reachability/response tests against each IP\n";
...
print " - SNMP (SysName and SysDescr)\n";
...
foreach $x (@ARGV)
{
	...
	{
		$goodpar = "true";
		print "Usage: perl $PROGRAM_NAME in=infile out=outfile pwd=comstr [verbose]\n\n";
		...
		print "verbose: Optional argument to output details about each test to the console, \n";
		print "          by default only progress info is output to the console and details to a log\n\n";
...
until (-e $InFile)
{
	print "The input file name you specified doesn't exists, please enter correct file name with complete path if nessisary:";
	$InFile = <STDIN>;
	chomp $InFile;
	$InFile =~ s/\\/\//g;	
}
...
#		$Q3 = $subnetquads[3] + $i;
  	...
  	else
  	{
		...
		{
			logentry ("Issuing a SNMP get for sysname ... \n",1);
		  ...
		  else
		  {
				...
				$InUse = 1;
				logentry ("Issuing a SNMP get for sysDescr ... \n",1);
