...
use strict;
my ($TelnetTimeout, $SNMPTimeout, %SMTPHosts, %PortTestArray, $from, $to, $subject);
...
my ($comstr, $session, $error, $sysname, $sysDescr, $result, $logfile, $errmsg, $progname, $t, $quad);
my ($sname, $sDescr, $strOut, $InFile, $outfile, $IPAddr, $line, $bError, $alive, $PortTest, $SSHTest, $x);
...
while (($key,$value) = each %PortTestArray) 
{
...
$strOut = "IP\tDNS_Name\tSysName\tType\tMake\tPing\t";
while (($key,$value) = each %PortTestArray) 
{
	$strOut .=  "$value\t";
}
...
$timeEst = ($PortTestCount * ($TelnetTimeout + 1)) + (2 * $SNMPTimeout) + 10;
...
logentry ("plus two SNMP tests with a timeout value of $SNMPTimeout seconds and 10 second timeout on ping\n",0);
...
foreach $line (@inArray)
{
	...
	for ($i = $subnetquads[3]; $i <= $SubnetEnd; $i++) 
	{
  	...
  	else
  	{
		...
		$strOut = "$IPAddr\t$DNSname\t$sname\t$devType\t$Make\t$alive\t";
		while (($key,$value) = each %PortTestResults) 
		{
			$strOut .=  "$value\t";
		}
		$strOut .= "$sDescr\n";
