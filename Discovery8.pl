...
#  		$InUse = 1;
		...
		else
		{
			$error = "";
			$errmsg = "";
			$value = $getOIDs{"sysName"};
			logentry ("verifying SNMP connection with sysname get\n",2,1);
			$result = $session->get_request($value);
		 	if (!defined($result)) 
		 	{
				...
				logentry ("$errmsg\n",2,1);
				$SNMPTest = "Fail";
		 		logentry ("First attempt failed, trying again with all uppercase comstr\n",2,1);
		 		$session->close;
				($session,$error) = Net::SNMP->session(hostname => $IPAddr, community => uc($comstr), timeout => $SNMPTimeout);
				if (!defined($session)) 
				{
					$errmsg = sprintf("Connect ERROR: %s.", $error);
					logentry ("$errmsg\n",2,1);
		 		...
		 		else
		 		{
					...
					if (!defined($result)) 
					{
		 				...
		 				logentry ("$errmsg\n",2,1);
	   				$SNMPTest = "Fail";
	   				logentry ("second attempt failed, no further SNMP attempts for this device\n",2,1);
	   			}
	   			else
	   			{
	   				$SNMPTest = "Success";
	   				logentry ("second attempt succeeded, proceeding to pull SNMP data\n",2,1);	   				
	   			}
	   		}
		 	}
			
			if ($SNMPTest eq "Success")
			{
