...
my (%outhash,%ipaddHash,%ipaddout, %IPAddrOIDs, $iSpeed);
...
$IPAddrOIDs{"ipAdEntIfIndex"} = "1.3.6.1.2.1.4.20.1.2";
$IPAddrOIDs{"ipAdEntAddr"}    = "1.3.6.1.2.1.4.20.1.1";
$IPAddrOIDs{"ipAdEntNetMask"} = "1.3.6.1.2.1.4.20.1.3";
...
#			last;
	...
	foreach $Tablekey (sort keys %IPAddrOIDs)  
	{
		$value = $IPAddrOIDs{$Tablekey};
		logentry("Issuing a SNMP walk for $Tablekey ... \n",2,1);
		$result = $session->get_table($value);
		if (!defined($result)) 
		{
...
#			last;
		...
		else
		{
			...
			foreach $reskey(sort(keys %reshash)) 
			{
				...
				$OIDResult =~ s/\'//g;
		 		logentry ("OIDResultClean: $OIDResult\n",4,3);
				$ipaddout{$id}{$Tablekey} = $OIDResult;
			}
		}
	}
	
	foreach $Tablekey (sort keys %ipaddout)
	{
		$ipaddHash{$ipaddout{$Tablekey}{ipAdEntIfIndex}}{"IP"} = $ipaddout{$Tablekey}{ipAdEntAddr};
		$ipaddHash{$ipaddout{$Tablekey}{ipAdEntIfIndex}}{"Mask"} = $ipaddout{$Tablekey}{ipAdEntNetMask};
#		logentry ("PortHash: $outPorts{$Tablekey}{Portid} = $outPorts{$Tablekey}{PortName}\n",4,3);
	}

	logentry("deleting old interface results from database ... \n",2,1);
	...
	{
		if ($outhash{$OutKey}{ifSpeed} =~ /^\d+$/)
		{
			$iSpeed = 0;
		}
		else
		{
			$iSpeed = $outhash{$OutKey}{ifSpeed};
		}
		$strSQL = "INSERT INTO tblInterfaces (iDeviceID, iIndex, vcInterface, iSpeed, vcMacAddr, vcAdminStatus, vcOpStatus, ";
		$strSQL .= "vcIPAddr, vcMask, dtUpdatedAt) ";
		$strSQL .= "VALUES ('$iDevID' , '$OutKey', '$outhash{$OutKey}{ifDesc}', '$iSpeed', '$outhash{$OutKey}{ifPhysAddress}',";
		$strSQL .= "'$AdminStatusCode{$outhash{$OutKey}{ifAdminStatus}}', '$OperStatusCode{$outhash{$OutKey}{ifOperStatus}}', ";
		$strSQL .= "'$ipaddHash{$OutKey}{\"IP\"}', '$ipaddHash{$OutKey}{\"Mask\"}', NOW());";
		logentry ("$strSQL\n",3,2);
		eval 
		{
			$dbh->do($strSQL);
