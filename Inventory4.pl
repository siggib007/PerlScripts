...
my (@Inv, %invitem, $inst, $type, $loc, $len, %PhysClass, %FRU, $dev, $dstTable, $StartTime, $isec, $imin, $DC);
...
my ($to, $from, $subject, @body, $relay, %FlashDev, $Flashrs, $FlashCmdText, %hComStr, @body);
...
{	
	...
	$DC =  $SrcRS->{fields}{vcDCName}->{value};
	print "Device: $Device\t";
	if ($Device =~ m/sch/i)
	{
		print "Search Device\t";
		...
		else
		{
			...
			foreach $key(sort(keys %cdpcach))
			{
				...
				if (Win32::OLE->LastError() != 0 ) { cleanup("error while adding record to cdp table\n".Win32::OLE->LastError(),1);}
					...
					$CDPrs->{fields}{vcDCName}->{value}     = $DC;
		...
		else
		{
			...
			{		
				...
				if (Win32::OLE->LastError() != 0 ) { cleanup("error while adding record to inv table\n".Win32::OLE->LastError(),1);}			
					...
					$dstRS->{fields}{Datacenter}->{value}     = $DC;
