...
my (%getOIDs, $value, $errmsg, %outhash, $Tablekey, $reskey, $devname, $OutKey, $sth, $iDevID );
...
$tblName = "tblDevices";
$Where   = "vcSNMP = 'success'";
$DBHost  = "localhost";
$DBName  = "inventory";
$DBUser  = "script";
$DBpwd   = "test123";

$getOIDs{"cyACSDevId"}     = "1.3.6.1.4.1.2925.4.1.8";
$getOIDs{"cyACSpname"}     = "1.3.6.1.4.1.2925.4.1.1";
$getOIDs{"cyACSversion"}   = "1.3.6.1.4.1.2925.4.1.1";
$getOIDs{"cyACSFlashSize"} = "1.3.6.1.4.1.2925.4.1.5";
$getOIDs{"cyACSRAMSize"}   = "1.3.6.1.4.1.2925.4.1.6";
...
$strSQL .= "VALUES ('$strUUID',now(),'$host','$ShortName',$ScriptInst,$NumScripts,$iDevCount,now());";
logentry ("$strSQL\n",3,2);
$dbh->do($strSQL);
...
#			last;
	...
	foreach $OutKey (sort keys %outhash)
	{
		...
		$strSQL .= "vcRamSize='$outhash{cyACSRAMSize}' where iDeviceID = $iDevID;";
		logentry ("$strSQL\n",3,2);
		$dbh->do($strSQL);
	}
	$session->close;
